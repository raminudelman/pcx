...
//              rank id equals to 'peer'.
//
int ring_exchange(void *comm, volatile void *send_buf_left,
                  volatile void *send_buf_right, volatile void *recv_buf_left,
                  volatile void *recv_buf_right, size_t size,
                  uint32_t peer_left, uint32_t peer_right, uint32_t tag1,
                  uint32_t tag2);

template <typename T> class PcxAllreduceChunkedRing {
  public:
    PcxAllreduceChunkedRing(const int contextSize, const int contextRank,
    ...
    void connect_and_prepare() {
        ...
        // step_count holds the number of iterations that the ring algorithm
        // should
        // perform in reduce-scatter stage, and in all-gather stage.
        // During reduce-scatter it will perform step_count iterations,
        // and in all-gather stage it will perform additional step_count
        // iterations.
        ...
        pipeline_ = contextSize_ * 2; // TODO: What is this? it overrides the
                                      // loop that was before!!

        // The tmpMem will be used for "incoming" messages from the qps, this
        // buffer is of size pipeline and each element in the buffer is with
        // size peicesize.
        ...
        rd_.ring_qps = new RingQps(&ring_exchange, comm, myRank, contextSize_,
                                   tag1, tag2, mem_.tmpMem, ibv_ctx_);
        ...
        for (unsigned step_idx = 0; step_idx < step_count; step_idx++) {
            ...
            if (step_idx > 0) {
                rd_.iters[step_idx].umr_iov.push_back(new RefMem(
                    mem_.tmpMem->next())); // The next() operation is cyclic.
            }
            rd_.iters[step_idx].outgoing_buf =
                new UmrMem(rd_.iters[step_idx].umr_iov, ibv_ctx_);
        }
        ...
        for (unsigned step_idx = 0; step_idx < step_count; step_idx++) {
            ...
            if (credits == 1) {
                // reduce_write with 'require_cmpl==true' means that we perform
                // reduce and perform RDMA write to the next rank and require a
                // completion for the RDMA write.
                sess->reduce_write(right, rd_.iters[step_idx].outgoing_buf,
                                   step_idx, num_vectors,
                                   MLX5DV_VECTOR_CALC_OP_ADD,
                                   MLX5DV_VECTOR_CALC_DATA_TYPE_FLOAT32, true);
                ...
                // reduce_write with 'require_cmpl==false' means that we perform
                // reduce and perform RDMA write to the
                // next rank. The RDMA write will send the outgoing_buf to the
                // incoming
            ...
            // Once a rank sent a message from the sending QP (to the rank to
            // the right),
            // the rank knows it should wait for the message from the left QP
            // the
        ...
        for (unsigned step_idx = 0; step_idx < step_count; step_idx++) {
            ...
            RefMem newVal(
                (*mem_.tmpMem)[last_frag]); // Cyclic pipe...there is wraparound
            ...
            if (credits == 1) {
                ...
                for (uint32_t buf_idx = 0; buf_idx < vectors_to_reduce;
                     buf_idx++) {
                    sess->write(lqp, &newVal,
                                rd_.iters[step_idx].umr_iov[buf_idx], false);
                }
                sess->wait(right, true);
                sess->wait(lqp); // Waiting for the receive to finish in the
                                 // loopback QP
        ...
        // Making the NIC wait for the last credit although the user already got
        // the reduce result from the lpq because in the run, we poll only the
        // lpq.
    ...
    void debug_hang_report(std::string str, int count = 0) {
        ...
        fprintf(stderr,
                "======================================================\n");
        fprintf(stderr,
                "Waiting for Loopback QP CQE with index %d to be completed\n",
...
// rd_.pqp->left->print();
    ...
    void print_values(volatile float *buf,
  ...
  private:
    ...
    typedef struct mem_registration_ring { // TODO: Convert into a class and
                                           // delete from pcx_mem.h all the Iop*
                                           // functions and typdefs
    ...
    class StepCtx {
      public:
        ...
        Iov umr_iov; // Iov == Input/Output Vector, UMR is because the user's
                     // buffer is not contigious and we convert it to a UMR.
        NetMem *outgoing_buf; // The buffer which contains the result of the
                              // reduce and which will be sent to the peer rank
    };
    ...
    typedef struct rd_connections_ring {
        ...
        // Each element in the array holds all the data structure that the
        // algorithm
