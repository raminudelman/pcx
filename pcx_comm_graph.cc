...
void CommGraph::wait(PcxQp *slave_qp, bool wait_scq) {
    ...
    QP_PRINT("Add graph operation: ManagementQP        will 'wait'             "
             "on QP   ID='%d' on SQ? %d. \n",
...
void CommGraph::write(RingQp *slave_qp, NetMem *local, size_t pos,
                      bool require_cmpl) {
...
void CommGraph::write(LoopbackQp *slave_qp, NetMem *local, RefMem *remote,
                      bool require_cmpl) {
    LambdaInstruction lambda = slave_qp->write(local, remote, require_cmpl);
    this->enqueue(lambda);
    QP_PRINT("Add graph operation: LoopbackQp  ID='%d' will 'write'            "
             "to peer ID=%d \n",
             slave_qp->id, slave_qp->get_pair()->id);
    lambda = this->mqp->cd_send_enable(slave_qp);
    this->enqueue(lambda);
}
void CommGraph::write(LoopbackQp *slave_qp, NetMem *local, NetMem *remote,
                      bool require_cmpl) {
    LambdaInstruction lambda = slave_qp->write(local, remote, require_cmpl);
    this->enqueue(lambda);
    QP_PRINT("Add graph operation: LoopbackQp  ID='%d' will 'write'            "
...
void CommGraph::write(DoublingQp *slave_qp, NetMem *local, NetMem *remote,
                      bool require_cmpl) {
    LambdaInstruction lambda = slave_qp->write(local, remote, require_cmpl);
    this->enqueue(lambda);
    QP_PRINT("Add graph operation: DoublingQp  ID='%d' will 'write'            "
             "to peer ID=%d \n",
             slave_qp->id, slave_qp->get_pair()->id);
    lambda = this->mqp->cd_send_enable(slave_qp);
    this->enqueue(lambda);
...
void CommGraph::send_credit(TransportQp *slave_qp) {
    ...
    this->enqueue(lambda);
    QP_PRINT("Add graph operation: TransportQP ID='%d' will 'send_credit'      "
             "to peer ID=%d \n",
             slave_qp->id, slave_qp->get_pair()->id);
    lambda = this->mqp->cd_send_enable(slave_qp);
    this->enqueue(lambda);
